 ...
 */
bool handle_client_request(int sockfd, char *buff, int received_bytes, bool is_logged_in)
{
    buff[strcspn(buff, "\r\n")] = '\0';
...
void *communicate(void *arg)
{
    ...
    while (1)
    {
        ...
        if (disconnect == 0)
            is_logged_in = handle_client_request(sockfd, mess, strlen(mess), is_logged_in);
        else
            break;
    }
